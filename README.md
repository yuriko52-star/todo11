...
##  カテゴリーと結びつけた Todo の追加  
1. リレーションで紐づいたカテゴリの取得(TodoController)
withメソッド  
モデルで作成したメソッドを使用し、テーブルの情報とテーブルに紐づいているテーブルの情報をまとめて取得することができる。 今回はtodoモデルで作成したcategoryメソッドを使用。  
2. Viewファイルの編集 
3. コントローラーの設定  
 category_idとcontentの内容をデータベースに保存するように設定  
